...
type Cache interface { Get(key string) ([]byte, bool); Set(key string, val []byte, ttl time.Duration) }

type memory struct{ mu sync.Mutex; m map[string]entry }
type entry struct{ b []byte; exp time.Time }
...
func (c *memory) Get(key string) ([]byte, bool) {
    ...
    e, ok := c.m[key]
    if !ok || (!e.exp.IsZero() && time.Now().After(e.exp)) { return nil, false }
    return e.b, true
}
func (c *memory) Set(key string, val []byte, ttl time.Duration) {
    c.mu.Lock(); defer c.mu.Unlock()
    e := entry{b: append([]byte(nil), val...)}
    if ttl > 0 { e.exp = time.Now().Add(ttl) }
    c.m[key] = e
}
