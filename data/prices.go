...
func (p *Prices) Klines(ctx context.Context, venue, symbol, window string, limit int) ([]float64, []float64, error) {
    if strings.ToLower(venue) != "binance" { return nil, nil, fmt.Errorf("only binance supported in slice") }
    ...
    if b, ok := p.c.Get(key); ok {
        var out struct{ C []float64; V []float64 }
        if json.Unmarshal(b, &out) == nil { return out.C, out.V, nil }
    }
    ...
    var rows [][]any
    if err := json.NewDecoder(resp.Body).Decode(&rows); err != nil { return nil, nil, err }
    ...
    b, _ := json.Marshal(struct{ C []float64; V []float64 }{C: close, V: vols})
