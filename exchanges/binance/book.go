...
type BookProvider interface { Metrics(ctx context.Context, symbol string) (OrderbookMetrics, error) }
...
func (b *book) run() {
    ...
    for {
        ...
        for {
            ...
            t0 := time.Now()
            var raw struct{ Bids [][]string `json:"b"`; Asks [][]string `json:"a"` }
            if err := json.Unmarshal(msg, &raw); err != nil { continue }
...
func (b *book) refreshSnapshot() {
    ...
    var out struct{ Bids [][]string `json:"bids"`; Asks [][]string `json:"asks"` }
...
func parseLevels(x [][]string) []level {
    ...
    for _, e := range x {
        if len(e) < 2 { continue }
        var p, q float64
        fmt.Sscan(e[0], &p); fmt.Sscan(e[1], &q)
        if q <= 0 { continue }
...
func (b *book) applyDiff(m struct{ Bids, Asks [][]string }) {
    ...
    if len(m.Bids) > 0 { b.bids = parseLevels(m.Bids) }
    if len(m.Asks) > 0 { b.asks = parseLevels(m.Asks) }
}
...
func (b *book) metrics() OrderbookMetrics {
    ...
    if len(b.bids) == 0 || len(b.asks) == 0 { return OrderbookMetrics{Source:"binance"} }
    // best bid/ask
    bestBid := b.bids[0].P
    bestAsk := b.asks[0].P
    ...
    depthUSD := 0.0
    // bids (<= mid, >= low)
    for _, lv := range b.bids { if lv.P < low { break }; depthUSD += lv.P * lv.Q }
    // asks (>= mid, <= high)
    for _, lv := range b.asks { if lv.P > high { break }; depthUSD += lv.P * lv.Q }
    // p99 latency
    lat := append([]int64(nil), b.lat...)
    sort.Slice(lat, func(i,j int) bool { return lat[i] < lat[j] })
    p99 := int64(0)
    if n := len(lat); n > 0 { p99 = lat[int(math.Ceil(float64(n)*0.99))-1] }
