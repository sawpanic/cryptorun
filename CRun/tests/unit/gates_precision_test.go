...
func TestComputeSpreadBps_WithRounding(t *testing.T) {
	testCases := []struct {
		...
		{"49.5 bps rounds to 50", 100.0, 100.495, 50},
		{"50.5 bps rounds to 51", 100.0, 100.505, 51},
		{"Very tight spread", 50000.0, 50000.25, 5}, // 5 bps
...
func TestSpreadGate_InclusiveThreshold(t *testing.T) {
	...
	testCases := []struct {
		...
		{"49.5 bps rounds to 50 - PASS", 100.0, 100.495, true, "half-up at boundary"},
		{"50.5 bps rounds to 51 - FAIL", 100.0, 100.505, false, "half-up over boundary"},
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if !math.IsNaN(tc.depth) && !math.IsInf(tc.depth, 0) {
				if tc.depth <= 0 {
					expectedValue = 0 // GuardFinite fallback
				}
