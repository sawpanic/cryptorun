...
// NewGateEvaluator creates a Pre-Movement v3.3 gate evaluator with tiered microstructure
func NewGateEvaluator(microEvaluator microstructure.Evaluator, config *GateConfig) *GateEvaluator {
	if config == nil {
		config = DefaultGateConfig()
	}

	// Initialize tiered gate calculator for consultative microstructure checks
...
type ConfirmationResult struct {
	...
	MicroReport       *microstructure.EvaluationResult `json:"micro_report"`        // Legacy microstructure evaluation
	TieredMicroReport *microstructure.TieredGateResult `json:"tiered_micro_report"` // NEW: Tiered microstructure evaluation
...
func (ge *GateEvaluator) EvaluateConfirmation(ctx context.Context, data *ConfirmationData) (*ConfirmationResult, error) {
	...
	microResult, err := ge.microEvaluator.EvaluateSnapshot(data.Symbol)
	if err != nil {
		result.Warnings = append(result.Warnings, fmt.Sprintf("Legacy microstructure evaluation failed: %v", err))
	} else {
		result.MicroReport = &microResult
	}

	// NEW: Evaluate tiered microstructure as additional consultation
	// Mock ADV calculation from volume data (in production would come from market data)
	estimatedADV := data.VolumeRatio24h * 1000000.0 // Mock ADV estimation
	vadrInput := &microstructure.VADRInput{
		High:         data.SpreadBps * 1.1, // Mock approximation
		Low:          data.SpreadBps * 0.9,
		Volume:       estimatedADV / 100.0,
		ADV:          estimatedADV,
		CurrentPrice: 50000.0, // Mock price
	}

	tieredResult, err := ge.tieredCalculator.EvaluateTieredGates(ctx, data.Symbol, estimatedADV, vadrInput)
	if err != nil {
		result.Warnings = append(result.Warnings, fmt.Sprintf("Tiered microstructure evaluation failed: %v", err))
	} else {
		result.TieredMicroReport = tieredResult

		// Add tiered microstructure insights to warnings (consultative)
		if !tieredResult.AllGatesPass {
			result.Warnings = append(result.Warnings,
				fmt.Sprintf("Tiered microstructure concern: %s (%s tier)",
					tieredResult.RecommendedAction, tieredResult.Tier.Name))
		}

		// Add venue degradation warnings
		if tieredResult.DegradedMode {
			result.Warnings = append(result.Warnings,
				fmt.Sprintf("Operating in degraded mode - primary venue: %s", tieredResult.PrimaryVenue))
		}
	}
