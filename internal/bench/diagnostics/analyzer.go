...
import (
	...
	"cryptorun/internal/bench/sources"
)
...
func NewDiagnosticsAnalyzer(config DiagnosticsConfig) *DiagnosticsAnalyzer {
	return &DiagnosticsAnalyzer{
		...
		// regimeDetector: regime.NewRegimeDetector(regime.DefaultConfig()),
	}
}
...
func (da *DiagnosticsAnalyzer) AnalyzeBenchmarkResults(ctx context.Context, topGainers map[string][]TopGainer, scanResults map[string][]string) (*DiagnosticReport, error) {
	...
	currentRegime, err := da.regimeDetector.DetectCurrentRegime(ctx)
	if err != nil {
		currentRegime = "trending" // Default fallback
	}
...
func (da *DiagnosticsAnalyzer) simulateCompliantPnL(ctx context.Context, symbol, window, regime string) float64 {
	...
	entryBar := da.findCompliantEntry(priceData, config)
	if entryBar == -1 {
	...
	exitBar := da.findEarliestExit(priceData, entryBar, config)
...
// func (da *DiagnosticsAnalyzer) getRegimeAwareConfig(regime string) momentum.MomentumConfig {
// 	return momentum.DefaultConfig() // Mock - should return regime-specific config
// }

// func (da *DiagnosticsAnalyzer) findCompliantEntry(priceData []sources.PriceBar, config momentum.MomentumConfig) int {
// 	return 1 // Mock - should find first compliant entry bar
// }

// func (da *DiagnosticsAnalyzer) findEarliestExit(priceData []sources.PriceBar, entryBar int, config momentum.MomentumConfig) int {
// 	return len(priceData) - 1 // Mock - should find earliest exit condition
// }
