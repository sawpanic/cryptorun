...
// MomentumRegimeWeights defines momentum weighting by market regime
type MomentumRegimeWeights struct {
...
var (
	BullRegimeWeights = MomentumRegimeWeights{
		Regime: "bull",
	...
	ChoppyRegimeWeights = MomentumRegimeWeights{
		Regime: "choppy",
		Weights: map[TimeFrame]float64{
			TF1h:  0.15, // 15%
			TF4h:  0.25, // 25%
	...
	HighVolRegimeWeights = MomentumRegimeWeights{
		Regime: "high_vol",
		Weights: map[TimeFrame]float64{
			TF1h:  0.10, // 10%
			TF4h:  0.20, // 20%
...
func (mc *MomentumCalculator) GetRegimeWeights() MomentumRegimeWeights {
	switch mc.regime {
	case "bull":
