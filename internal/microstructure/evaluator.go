...
// NewEvaluator creates a new microstructure evaluator (compatibility function)
func NewEvaluator() *MicrostructureEvaluator {
	return NewMicrostructureEvaluator(nil)
}
...
// EvaluateSnapshot performs snapshot evaluation for entry gates
func (me *MicrostructureEvaluator) EvaluateSnapshot(symbol string) (EvaluationResult, error) {
	// This is a mock implementation for now
	// In a real implementation, this would fetch real market data and check actual gates

	// Mock core metrics with reasonable values
	result := EvaluationResult{
		SpreadBps: 45.0,     // Mock spread in basis points
		DepthUSD:  125000.0, // Mock depth in USD
		VADR:      1.8,      // Mock VADR ratio
		Healthy:   true,     // Mock healthy status
	}

	// Simple health check based on mock thresholds
	result.Healthy = result.SpreadBps < 50.0 &&
		result.DepthUSD > 100000.0 &&
		result.VADR > 1.75

	return result, nil
}

// EvaluateSnapshotWithContext performs snapshot evaluation with context (legacy compatibility)
func (me *MicrostructureEvaluator) EvaluateSnapshotWithContext(ctx context.Context, symbol string) (*SnapshotResult, error) {
	// Legacy method for backward compatibility
	return &SnapshotResult{
		VADR:            1.8,              // Mock VADR
		SpreadBps:       45.0,             // Mock spread
		DepthUSD:        125000.0,         // Mock depth
		DailyVolumeUSD:  750000.0,         // Mock daily volume
		BarCount:        25,               // Mock bar count
		BarsFromTrigger: 1,                // Mock freshness
		LateFillDelay:   15 * time.Second, // Mock late fill delay
		ADX:             27.0,             // Mock ADX
		Hurst:           0.58,             // Mock Hurst
		Timestamp:       time.Now(),
