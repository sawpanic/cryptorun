...
type MomentumTestDataProvider struct {
	mockData map[string]map[pipeline.TimeFrame][]pipeline.MarketData
}

func NewMomentumTestDataProvider() *MomentumTestDataProvider {
	return &MomentumTestDataProvider{
		mockData: make(map[string]map[pipeline.TimeFrame][]pipeline.MarketData),
	}
}

func (t *MomentumTestDataProvider) SetMockData(symbol string, tf pipeline.TimeFrame, data []pipeline.MarketData) {
...
func (t *MomentumTestDataProvider) GetMarketData(ctx context.Context, symbol string, timeframe pipeline.TimeFrame, periods int) ([]pipeline.MarketData, error) {
...
func TestMomentumCalculation(t *testing.T) {
	provider := NewMomentumTestDataProvider()
